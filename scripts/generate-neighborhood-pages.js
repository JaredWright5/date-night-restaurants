...
// Get all neighborhoods from Supabase
const { data: neighborhoods } = await supabase
  .from('neighborhoods')
  .select('name, slug')
  .order('name');

console.log('Generating neighborhood pages for:', neighborhoods.map(n => n.name).join(', '));

// Template for neighborhood pages
const generateNeighborhoodPage = (neighborhood) => {
  return `---
import BaseLayout from '@/layouts/BaseLayout.astro';
import RestaurantCard from '@/components/RestaurantCard.astro';
import { getRestaurantsByNeighborhood } from '@/lib/supabase';

// Get ${neighborhood.name} restaurants from Supabase
let restaurants;
try {
  restaurants = await getRestaurantsByNeighborhood('${neighborhood.slug}');
} catch (error) {
  console.error('Error fetching ${neighborhood.name} restaurants:', error);
  return new Response('Database connection failed. Please check your Supabase configuration.', { status: 500 });
}

// Generate page title and description
const pageTitle = \`Date Night Restaurants in ${neighborhood.name} | \${restaurants.length} Romantic Dining Spots\`;
const pageDescription = \`Discover the best date night restaurants in ${neighborhood.name}, Los Angeles. \${restaurants.length} romantic dining spots perfect for couples, from cozy neighborhood spots to upscale fine dining.\`;
  ...
  <div class="restaurants-page">
    <div class="container">
      ...
      <header class="page-header">
        <h1>Date Night Restaurants in ${neighborhood.name}</h1>
        <p class="page-description">
          Discover romantic dining spots in ${neighborhood.name}, Los Angeles
        </p>
      ...
      <section class="restaurants-list">
        <div class="container">
          <h2>Showing \${restaurants.length} Restaurants in ${neighborhood.name}</h2>
...
for (const neighborhood of neighborhoods) {
  const pageContent = generateNeighborhoodPage(neighborhood);
  const filePath = join(neighborhoodsDir, `${neighborhood.slug}.astro`);
  writeFileSync(filePath, pageContent);
  console.log(`Generated: ${neighborhood.name} -> ${neighborhood.slug}.astro`);
}
